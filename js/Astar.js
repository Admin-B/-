    ...
    while( 1 ){
      if(openset.length > 0){
        ...
        for(var i=0; i<openset.length; i++){
          if(openset[i].t<current.t){
            current=openset[i];
          }
        }
        if(current.prev){
          for(var i=0; i<current.neighbors.length; i++){
            var c=current.neighbors[i];
            if(closedset.includes(c) && current.prev!==c && current.t>c.t){
              current.prev=c;
              current.t=c.t;
              if(c.prev && (c.y-c.prev.y!=current.y-c.y || c.x-c.prev.x!=current.x-c.x)){
                current.t+=1;
              }
            }
          }
        ...
        for(var i=0; i<neighbors.length; i++){
          ...
          n.t=current.t;
          if(current.prev && (current.y-current.prev.y!=n.y-current.y || current.x-current.prev.x!=n.x-current.x)){
            n.t+=1;
          }
